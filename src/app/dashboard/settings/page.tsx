...
export default async function SettingsPage() {
  ...
  // Fetch full partner record for branding form
  const admin = createAdminClient();
  const { data: partner } = await admin
    .from("partners")
    .select(
      "name, slug, primary_color, accent_color, support_email, support_phone, custom_domain"
    )
    .eq("id", account.id)
    .maybeSingle();
    ...
    <div className="max-w-5xl mx-auto px-6 md:px-10 py-8 space-y-8">
      ...
      <section className="glass-panel rounded-2xl border border-outline-variant/15 p-6">
        ...
        <p className="text-xs text-on-surface-variant/60 mb-3">
          We use Resend to notify you when a client submits. Send a test message
