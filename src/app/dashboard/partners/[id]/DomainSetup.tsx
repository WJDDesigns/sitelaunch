...
export default function DomainSetup({ partnerId, domain, cnameTarget = VERCEL_CNAME }: Props) {
  ...
  function handleVerify() {
    startTransition(async () => {
      const res = await verifyDomainAction(partnerId, domain);
      setResult(res);
    });
  }
  ...
  return (
    <div className="glass-panel rounded-2xl border border-outline-variant/15 p-6 space-y-5">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs font-mono text-on-surface-variant/60">{domain}</span>
      </div>

      {/* Steps */}
      <div className="space-y-4">
        <p className="text-sm text-on-surface-variant leading-relaxed">
          To use <span className="font-semibold text-on-surface">{domain}</span> as your
          ...
          <li className="flex gap-3">
            ...
            <div>
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td className="px-4 py-2.5 font-mono text-on-surface">{domain.split(".")[0]}</td>
              ...
              <p className="text-on-surface-variant/60 text-xs mt-1.5">
                If your provider asks for a hostname, enter <span className="font-mono text-on-surface">{domain.split(".")[0]}</span>.
