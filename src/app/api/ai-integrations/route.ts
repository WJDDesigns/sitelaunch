...
export async function GET() {
  try {
    ...
    console.error("[ai-integrations] GET error:", err);
    return NextResponse.json({ error: "Failed to load AI integrations" }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { provider, apiKey, modelPreference } = body;
    ...
    const admin = createAdminClient();
    const { error } = await admin.from("ai_integrations").upsert(
      {
    ...
    if (error) {
      console.error("[ai-integrations] upsert error:", error);
      return NextResponse.json({ error: "Failed to save integration" }, { status: 500 });
    }

    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error("[ai-integrations] POST error:", err);
    return NextResponse.json({ error: "Failed to save AI integration" }, { status: 500 });
  }
}
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    if (error) {
      console.error("[ai-integrations] delete error:", error);
      return NextResponse.json({ error: "Failed to remove integration" }, { status: 500 });
    }

    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error("[ai-integrations] DELETE error:", err);
    return NextResponse.json({ error: "Failed to remove AI integration" }, { status: 500 });
  }
}
